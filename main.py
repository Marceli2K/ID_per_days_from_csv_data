...
def download_data():
    """
    Download data from gov api and saving them to files.
    """
    csv_2022 = 'https://api.dane.gov.pl/media/resources/20220407/' \
               'DOWODY_2022_Tabela_do_Danych_Publicznych_I.csv'
    csv_2021 = 'https://api.dane.gov.pl/media/resources/20220114/' \
               'DOWODY_2021_Tabela_do_Danych_Publicznych_IV.csv'
    csv_2020 = 'https://api.dane.gov.pl/media/resources/20210112/' \
               'DOWODY_2020_Tabela_do_Danych_Publicznych_IV.csv'
    csv_2019 = 'https://api.dane.gov.pl/media/resources/20200616/' \
               'DOWODY_2019_Tabela_do_Danych_Publicznych.csv'
...
def data_for_poland(filename):
    sum_of_poland_1_quarter, sum_of_poland_2_quarter, \
    sum_of_poland_3_quarter, sum_of_poland_4_quarter = 0, 0, 0, 0
    cols = ['l.p', 'WOJEWÓDZTWO', 'I KWARTAŁ', 'II KWARTAŁ',
            'III KWARTAŁ', 'IV KWARTAŁ', 'ROK']

    df_read_csv_file = (pd.read_csv(filename, header=None, names=cols, encoding="utf8")  # open and check if has header
    [lambda x: np.ones(len(x)).astype(bool)
        if (x.iloc[0] != cols).all()
        else np.concatenate([[False], np.ones(len(x) - 1).astype(bool)])]
    )
    try:
        sum_of_poland_1_quarter = df_read_csv_file['I KWARTAŁ'].astype(int).sum()
    except:
        print("ERROR")
    try:
        sum_of_poland_2_quarter = df_read_csv_file['II KWARTAŁ'].astype(int).sum()

        sum_of_poland_3_quarter = df_read_csv_file['III KWARTAŁ'].astype(int).sum()

        sum_of_poland_4_quarter = df_read_csv_file['IV KWARTAŁ'].astype(int).sum()
    except:
        return sum_of_poland_1_quarter, 0, 0, 0

    return sum_of_poland_1_quarter, sum_of_poland_2_quarter,\
           sum_of_poland_3_quarter, sum_of_poland_4_quarter
...
def calculate_range(data_start, data_end):
    ...
    data = {
        "2019-Q1": q1_2019_data, '2019-Q2': q2_2019_data, '2019-Q3': q3_2019_data,
        '2019-Q4': q4_2019_data, '2020-Q1': q1_2020_data, '2020-Q2': q2_2020_data,
        '2020-Q3': q3_2020_data, '2020-Q4': q4_2020_data, '2021-Q1': q1_2021_data,
        '2021-Q2': q2_2021_data, '2021-Q3': q3_2021_data, '2021-Q4': q4_2021_data,
        '2022-Q1': q1_2022_data, '2022-Q2': q2_2022_data, '2022-Q3': q3_2022_data,
        '2022-Q4': q4_2022_data
    }
